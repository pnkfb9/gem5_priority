...
Wire::signal_fall_time ()
{
  ...
  rt = horowitz (0, timeconst, deviceType->Vth/deviceType->Vdd_cacti, deviceType->Vth/deviceType->Vdd_cacti, FALL) / (deviceType->Vdd_cacti - deviceType->Vth);
  timeconst = (drain_C_(g_tp.min_w_nmos_, NCH, 1, 1, g_tp.cell_h_def) +
      drain_C_(min_w_pmos, PCH, 1, 1, g_tp.cell_h_def) +
      gate_C(min_w_pmos + g_tp.min_w_nmos_, 0)) *
    tr_R_on(g_tp.min_w_nmos_, NCH, 1);
  ft = horowitz (rt, timeconst, deviceType->Vth/deviceType->Vdd_cacti, deviceType->Vth/deviceType->Vdd_cacti, RISE) / deviceType->Vth;
  return ft;
}
...
double Wire::signal_rise_time ()
{
  ...
  timeconst = (drain_C_(g_tp.min_w_nmos_, NCH, 1, 1, g_tp.cell_h_def) +
      ...
      gate_C(min_w_pmos + g_tp.min_w_nmos_, 0)) *
    tr_R_on(g_tp.min_w_nmos_, NCH, 1);
  rt = horowitz (0, timeconst, deviceType->Vth/deviceType->Vdd_cacti, deviceType->Vth/deviceType->Vdd_cacti, RISE) / deviceType->Vth;
  timeconst = (drain_C_(g_tp.min_w_nmos_, NCH, 1, 1, g_tp.cell_h_def) +
      drain_C_(min_w_pmos, PCH, 1, 1, g_tp.cell_h_def) +
      gate_C(min_w_pmos + g_tp.min_w_nmos_, 0)) *
    tr_R_on(min_w_pmos, PCH, 1);
  ft = horowitz (rt, timeconst, deviceType->Vth/deviceType->Vdd_cacti, deviceType->Vth/deviceType->Vdd_cacti, FALL) / (deviceType->Vdd_cacti - deviceType->Vth);
  return ft; //sec
}
...
#define RES_ADJ (8.6) // Increase in resistance due to low driving vol.
  ...
  delay = horowitz(inputrise, timeconst, deviceType->Vth/deviceType->Vdd_cacti,
      deviceType->Vth/deviceType->Vdd_cacti, RISE);
  double temp_power = cap_eq*deviceType->Vdd_cacti*deviceType->Vdd_cacti;

  inputrise = delay / (deviceType->Vdd_cacti - deviceType->Vth); /* for the next stage */
  ...
  delay += horowitz(inputrise, timeconst, deviceType->Vth/deviceType->Vdd_cacti,
      deviceType->Vth/deviceType->Vdd_cacti, FALL);
  temp_power += cap_eq*deviceType->Vdd_cacti*deviceType->Vdd_cacti;


  transmitter.delay = delay;
  transmitter.power.readOp.dynamic = temp_power*2; /* since it is a diff. model*/
  transmitter.power.readOp.leakage = 0.5 * deviceType->Vdd_cacti *
   ...
   */
  delay += horowitz(inputrise, timeconst, deviceType->Vth/deviceType->Vdd_cacti, .25, 0);
#define VOL_SWING .1
  ...
  l_wire.power.readOp.leakage = 0.5 * deviceType->Vdd_cacti*
    (4* simplified_nmos_leakage (nsize, g_ip->temp));


  //double rt = horowitz(inputrise, timeconst, deviceType->Vth/deviceType->Vdd_cacti,
  //    deviceType->Vth/deviceType->Vdd_cacti, RISE)/deviceType->Vth;
...
void Wire::delay_optimal_wire ()
{
  ...
  switching = (repeater_scaling * (input_cap + out_cap) +
      repeater_spacing * (wc/len)) * deviceType->Vdd_cacti * deviceType->Vdd_cacti;
...
#define Ishort_ckt 65e-6 /* across all tech Ref:Banerjee et al. {IEEE TED} */
  short_ckt = deviceType->Vdd_cacti * g_tp.min_w_nmos_ * Ishort_ckt * 1.0986 *
    repeater_scaling * tc;
