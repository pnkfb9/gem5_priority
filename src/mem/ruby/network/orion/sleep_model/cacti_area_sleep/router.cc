...
Router::tr_crossbar_power() {
  return (crossbar_inpline()*Vdd_cacti*Vdd_cacti*flit_size/2 + 
      crossbar_outline()*Vdd_cacti*Vdd_cacti*flit_size/2)*2;
}
...
void Router::buffer_stats() 
{
  ...
  dyn_p.is_dram = false;
  dyn_p.V_b_sense = Vdd_cacti; // FIXME check power calc.
