...
void Htree2::input_nand(double s1, double s2, double l_eff)
{
  ...
  delay+= horowitz (w1.out_rise_time, tc, 
      deviceType->Vth/deviceType->Vdd_cacti, deviceType->Vth/deviceType->Vdd_cacti, RISE);
  power.readOp.dynamic += 0.5 *
    (2*drain_C_(pton_size * nsize*min_w_pmos, PCH, 1, 1, g_tp.cell_h_def)
     ...
     + 2*gate_C(s2*(min_w_nmos + min_w_pmos), 0)) * 
    deviceType->Vdd_cacti * deviceType->Vdd_cacti;

  power.readOp.leakage += (wire_bw*cmos_Ileak(min_w_nmos*(nsize*2), min_w_pmos * nsize * 2, false)*deviceType->Vdd_cacti);
}
...
void Htree2::output_buffer(double s1, double s2, double l_eff)
{
  ...
  if(uca_tree) {
    power.readOp.leakage += (deviceType->Vdd_cacti*cmos_Ileak(min_w_nmos*(tr_size*2/*inverter + output tr*/ + size*6 /*nand + nor*/), 
