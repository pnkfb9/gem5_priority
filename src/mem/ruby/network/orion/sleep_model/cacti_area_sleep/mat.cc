  ...
  else
  {
    ...
    else
    {
      delay_bl_restore = bl_precharge_eq_drv->delay + 
        log((g_tp.sram.Vbitpre_cacti - 0.1 * dp.V_b_sense) / (g_tp.sram.Vbitpre_cacti - dp.V_b_sense))*
    ...
    bool is_dram)
{
  ...
  dynPower     += Ceq * g_tp.peri_global.Vdd_cacti * g_tp.peri_global.Vdd_cacti * tagbits * Ntbl;
  ...
  dynPower += (c_intrinsic + Cwire + c_gate_load) * g_tp.peri_global.Vdd_cacti * g_tp.peri_global.Vdd_cacti * tagbits * Ntbl;
  ...
  dynPower += Ceq * g_tp.peri_global.Vdd_cacti * g_tp.peri_global.Vdd_cacti * subarray.num_rows * Ntbl;
  ...
  dynPower += (c_intrinsic + Cwire + c_gate_load) * g_tp.peri_global.Vdd_cacti * g_tp.peri_global.Vdd_cacti;
...
double Mat::compute_bitline_delay(double inrisetime)
{
  ...
  if (is_dram == true)
  {
    V_b_pre = g_tp.dram.Vbitpre_cacti;
  ...
  { //SRAM
    V_b_pre = g_tp.sram.Vbitpre_cacti;
    v_th_mem_cell = g_tp.sram_cell.Vth;
    V_wl = g_tp.sram_cell.Vdd_cacti;
    ...
    leak_power_cc_inverters_sram_cell = Icell * g_tp.sram_cell.Vdd_cacti;
    leak_power_acc_tr_RW_or_WR_port_sram_cell = Iport * g_tp.sram_cell.Vdd_cacti;
  ...
  else
  {
    ...
    if (deg_bl_muxing > 1)
    {
      ...
      dynRdEnergy += (C_bl + 2 * C_drain_bit_mux) * 2 * dp.V_b_sense * g_tp.sram_cell.Vdd_cacti *
        subarray.num_cols * num_subarrays_per_mat;
      dynRdEnergy += (2 * C_drain_sense_amp_iso + C_sense_amp_latch +  C_drain_sense_amp_mux) * 
        2 * dp.V_b_sense * g_tp.sram_cell.Vdd_cacti * (subarray.num_cols * num_subarrays_per_mat / deg_bl_muxing);
      dynWriteEnergy += ((subarray.num_cols * num_subarrays_per_mat / deg_bl_muxing) / deg_senseamp_muxing) *
          num_act_mats_hor_dir * (C_bl + 2*C_drain_bit_mux) * g_tp.sram_cell.Vdd_cacti * g_tp.sram_cell.Vdd_cacti;
    }
    else
    {
      tau = (R_cell_pull_down + R_cell_acc) * 
        ...
        R_sense_amp_iso * (C_drain_sense_amp_iso + C_sense_amp_latch + C_drain_sense_amp_mux);
      dynRdEnergy += (C_bl + 2 * C_drain_sense_amp_iso + C_sense_amp_latch + C_drain_sense_amp_mux) *
        2 * dp.V_b_sense * g_tp.sram_cell.Vdd_cacti * subarray.num_cols * num_subarrays_per_mat;
      dynWriteEnergy += (((subarray.num_cols * num_subarrays_per_mat / deg_bl_muxing) / deg_senseamp_muxing) *
          num_act_mats_hor_dir * C_bl) * g_tp.sram_cell.Vdd_cacti * g_tp.sram_cell.Vdd_cacti;
...
double Mat::compute_sa_delay(double inrisetime)
{
  ...
  leak_power_sense_amps_closed_page_state = lkgIdlePh * g_tp.peri_global.Vdd_cacti * num_sa_subarray * num_subarrays_per_mat;
  leak_power_sense_amps_open_page_state   = lkgReadPh * g_tp.peri_global.Vdd_cacti * num_sa_subarray * num_subarrays_per_mat;
  ...
  delay_sa = tau * log(g_tp.peri_global.Vdd_cacti / dp.V_b_sense);
  power_sa.readOp.dynamic = C_ld * g_tp.peri_global.Vdd_cacti * g_tp.peri_global.Vdd_cacti * num_sa_subarray *
                            num_subarrays_per_mat;// * num_act_mats_hor_dir;
  power_sa.readOp.leakage = lkgIdle * g_tp.peri_global.Vdd_cacti;
...
double Mat::compute_subarray_out_drv(double inrisetime)
{
  ...
  power_subarray_out_drv.readOp.dynamic += C_ld * 0.5 * g_tp.peri_global.Vdd_cacti * g_tp.peri_global.Vdd_cacti;
  ...
  power_subarray_out_drv.readOp.leakage += cmos_Ileak(g_tp.min_w_nmos_, p_to_n_sz_r * g_tp.min_w_nmos_, is_dram) *
    0.5 * g_tp.peri_global.Vdd_cacti;
...
double Mat::compute_comparator_delay(double inrisetime)
{
  ...
  power_comparator.readOp.dynamic += 0.5 * Ceq * g_tp.peri_global.Vdd_cacti * g_tp.peri_global.Vdd_cacti * 4 * A; 
  ...
  double c1 = (tagbits_)*(drain_C_(g_tp.w_comp_n,NCH,1, 1, g_tp.cell_h_def, is_dram) +
    ...
    gate_C(WmuxdrvNANDn_cacti+WmuxdrvNANDp_cacti,0, is_dram);
  power_comparator.readOp.dynamic += 0.5 * c2 * g_tp.peri_global.Vdd_cacti * g_tp.peri_global.Vdd_cacti * 4 * A;
  power_comparator.readOp.dynamic += c1 * g_tp.peri_global.Vdd_cacti * g_tp.peri_global.Vdd_cacti *  (A - 1);
  ...
  double m = g_tp.peri_global.Vdd_cacti/nextinputtime;
  double Tcomparatorni;

  if((tstep) <= (0.5*(g_tp.peri_global.Vdd_cacti-g_tp.peri_global.Vth)/m)) 
  {
    double a = m;
    double b = 2*((g_tp.peri_global.Vdd_cacti*VTHEVALINV)-g_tp.peri_global.Vth);
    double c = -2*(tstep)*(g_tp.peri_global.Vdd_cacti-g_tp.peri_global.Vth)+1/m*((g_tp.peri_global.Vdd_cacti*VTHEVALINV)-g_tp.peri_global.Vth)*((g_tp.peri_global.Vdd_cacti*VTHEVALINV)-g_tp.peri_global.Vth);
    Tcomparatorni = (-b+sqrt(b*b-4*a*c))/(2*a);
  }
  else
  {
    Tcomparatorni = (tstep) + (g_tp.peri_global.Vdd_cacti+g_tp.peri_global.Vth)/(2*m) - (g_tp.peri_global.Vdd_cacti*VTHEVALINV)/m;
  }
  delay_comparator = Tcomparatorni+st1del+st2del+st3del;
  power_comparator.readOp.leakage = lkgCurrent * g_tp.peri_global.Vdd_cacti;
