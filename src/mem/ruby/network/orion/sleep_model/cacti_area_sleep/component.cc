...
Area Component::compute_return_gate_area(
    int gate_type,
    int num_inputs,
    double w_pmos, /* selected through optimization */
    double w_nmos, /* not used for ST equal to p:n=2:1(w_pmos/2), since we assume header ST */
    double h_gate) /* g_tp.cell_h_def =  50 * g_ip->F_sz_um; lambda rules*/
{
  Area gate;
