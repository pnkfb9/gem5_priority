...
Arbiter::compute_power() {
  power.readOp.dynamic =  (R*arb_req()*Vdd_cacti*Vdd_cacti/2 +
  R*arb_pri()*Vdd_cacti*Vdd_cacti/2 + 
      arb_grant()*Vdd_cacti*Vdd_cacti + arb_int()*0.5*Vdd_cacti*Vdd_cacti);
  double nor1_leak = deviceType->Vdd_cacti*cmos_Ileak(g_tp.min_w_nmos_*NTn1*2, min_w_pmos * PTn1*2, false); 
  double nor2_leak = deviceType->Vdd_cacti*cmos_Ileak(g_tp.min_w_nmos_*NTn2*R, min_w_pmos * PTn2*R, false); 
  double not_leak = deviceType->Vdd_cacti*cmos_Ileak(g_tp.min_w_nmos_*NTi, min_w_pmos * PTi, false); 
