...
void Decoder::compute_area()
{
  ...
  { // First check if this decoder exists
    ...
    power.readOp.leakage = cumulative_curr * g_tp.peri_global.Vdd_cacti;
...
double Decoder::compute_delays(double inrisetime)
{
  if (exist)
  {
    ...
    double Vdd_cacti = g_tp.peri_global.Vdd_cacti;
    ...
    {
      Vpp = g_tp.sram_cell.Vdd_cacti;
    }
    else
    {
      Vpp = g_tp.peri_global.Vdd_cacti;
    }
    ...
    power.readOp.dynamic += (c_load + c_intrinsic) * Vdd_cacti * Vdd_cacti;
    ...
    power.readOp.dynamic += c_load * Vpp * Vpp + c_intrinsic * Vdd_cacti * Vdd_cacti;
...
void PredecBlk::compute_area()
{
  if (exist)
  {
    ...
    power_nand2_path.readOp.leakage = leak_L1_nand2 * g_tp.peri_global.Vdd_cacti;
    power_nand3_path.readOp.leakage = leak_L1_nand3 * g_tp.peri_global.Vdd_cacti;
    power_L2.readOp.leakage         = leakage_L2    * g_tp.peri_global.Vdd_cacti; 
    ...
    pair<double, double> inrisetime)  // <nand2, nand3>
{
  ...
  if (exist)
  {
    ...
    if ((flag_two_unique_paths) || (number_inputs_L1_gate == 2))
    {
      ...
      power_nand2_path.readOp.dynamic += (c_load + c_intrinsic) * Vdd_cacti * Vdd_cacti;
      ...
      for (i = 1; i < number_gates_L1_nand2_path - 1; ++i)
      {
        ...
        delay_nand2_path += this_delay;
        inrisetime_nand2_path = this_delay / (1.0 - 0.5);
        power_nand2_path.readOp.dynamic += (c_intrinsic + c_load) * Vdd_cacti * Vdd_cacti;
      }
    ...
    { //Check if the number of gates in the first level is more than 1. 
      ...
      power_nand3_path.readOp.dynamic += (c_intrinsic + c_load) * Vdd_cacti * Vdd_cacti;
    ...
    if (flag_L2_gate)
    {
      if (flag_L2_gate == 2)
      {
        ...
        delay_nand2_path += this_delay;
        inrisetime_nand2_path = this_delay / (1.0 - 0.5);
        power_L2.readOp.dynamic += (c_intrinsic + c_load) * Vdd_cacti * Vdd_cacti;
      }
      else
      { // flag_L2_gate = 3
...
void PredecBlkDrv::compute_area()
{
  ...
  { // first check whether a predecoder block driver is needed
    ...
    power_nand2_path.readOp.leakage = leak_nand2_path * g_tp.peri_global.Vdd_cacti;
    power_nand3_path.readOp.leakage = leak_nand3_path * g_tp.peri_global.Vdd_cacti;
    ...
    double inrisetime_nand3_path)
{
  ...
  if (flag_driver_exists)
  {
    for (i = 0; i < number_gates_nand2_path - 1; ++i)
    {
      ...
      delay_nand2_path += this_delay;
      inrisetime_nand2_path = this_delay / (1.0 - 0.5);
      power_nand2_path.readOp.dynamic += (c_gate_load + c_intrinsic) * 0.5 * Vdd_cacti * Vdd_cacti;
    }
    ...
    if (number_gates_nand2_path != 0)
    {
      ...
      delay_nand2_path += this_delay;
      ret_val.first = this_delay / (1.0 - 0.5);  
      power_nand2_path.readOp.dynamic += (c_intrinsic + c_load) * 0.5 * Vdd_cacti * Vdd_cacti;
    }
    ...
    for (i = 0; i < number_gates_nand3_path - 1; ++i)
    {
      ...
      delay_nand3_path += this_delay;
      inrisetime_nand3_path = this_delay / (1.0 - 0.5);
      power_nand3_path.readOp.dynamic += (c_gate_load + c_intrinsic) * 0.5 * Vdd_cacti * Vdd_cacti;
    }
    ...
    if (number_gates_nand3_path != 0)
    {
      ...
      delay_nand3_path += this_delay;
      ret_val.second = this_delay / (1.0 - 0.5);  
      power_nand3_path.readOp.dynamic += (c_intrinsic + c_load) * 0.5 * Vdd_cacti * Vdd_cacti;
    }
  }
  return ret_val;
}
...
double Driver::compute_delay(double inrisetime)
{
  ...
  for (i = 0; i < number_gates - 1; ++i)
  {
    ...
    delay += this_delay;
    inrisetime = this_delay / (1.0 - 0.5);
    power.readOp.dynamic += (c_intrinsic + c_load) * g_tp.peri_global.Vdd_cacti * g_tp.peri_global.Vdd_cacti;
